...
import type { IDataToCreateUser, IDriver } from '@/lib/types'
...
export const FormEditDriver = ({ driver }: { driver: IDriver }) => {
  const defaultValues: IDriverToCreate = {
    names: driver.names,
    surnames: driver.surnames,
    phoneCode: driver.phone.split('-').slice()[0],
    phoneNumber: driver.phone.split('-').slice()[1],
    status: driver.status,
    ci: `${driver.ci.type}-${driver.ci.number}`,
    ciNumber: driver.ci.number,
    ciType: driver.ci.type.toLowerCase(),
    licenseType: driver.licenseType.toLowerCase()
  }

  const initialSecurityCertificateImageValues = {
    original: [{ data_url: driver.securityCertificateImage, file: null }],
    compressed: [{ data_url: driver.securityCertificateImage, file: null }]
  }

  const initialLicenseImageValues = {
    original: [{ data_url: driver.licenseImage, file: null }],
    compressed: [{ data_url: driver.licenseImage, file: null }]
  }

  const initialCIImageValues = {
    original: [{ data_url: driver.ci.image, file: null }],
    compressed: [{ data_url: driver.ci.image, file: null }]
  }

  const initialUserPhoneValues = {
    original: [{ data_url: driver.photo, file: null }],
    compressed: [{ data_url: driver.photo, file: null }]
  }

  const [securityCertificateImage, setSecurityCertificateImage] = useState(initialSecurityCertificateImageValues)
  ...
  const [licenseImage, setLicenseImage] = useState(initialLicenseImageValues)
  const [loading, setLoading] = useState({ meessage: '', value: false })
  const [userPhoto, setUserPhoto] = useState(initialUserPhoneValues)
  const [ciImage, setCIImage] = useState(initialCIImageValues)
