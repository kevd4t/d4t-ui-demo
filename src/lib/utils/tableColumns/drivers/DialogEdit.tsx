import { ICreateMeterModel as ICreateDriver, IMeterModel as IDriver } from '@/lib/types'
...
interface IDialogEditDriverProps {
  driver: IDriver
  isOpenEditModel: boolean
  setOpenEditModal: Dispatch<boolean>
}

export const DialogEditDriver = ({ driver, isOpenEditModel, setOpenEditModal }: IDialogEditDriverProps) => {
  const initialImageValues = {
    original: [{ data_url: driver.image, file: null }],
    compressed: [{ data_url: driver.image, file: null }]
  }

  const defaultValues: ICreateDriver = {
    title: driver.title,
    description: driver.description,
    isActive: driver.isActive.toString() as 'true' | 'false',
    type: driver.type
  }

  const [loading, setLoading] = useState({ meessage: '', value: false })
  const formDriver = useForm<ICreateDriver>({ defaultValues })
  ...
  const onSubmitFormMterModel = async (data: ICreateDriver) => {
  ...
  return (
    <Dialog open={isOpenEditModel} onOpenChange={setOpenEditModal}>
      <DialogContent>
        ...
        <form onSubmit={formDriver.handleSubmit(onSubmitFormMterModel)} autoFocus className='w-full'>
          <section className='w-full space-y-4'>
            <div className='w-full grid grid-cols-1 grid-rows-2 sm:grid-cols-2 sm:grid-rows-1 gap-y-3 gap-x-5'>
              <Input
                ...
                type='text'
                value={driver.title}
                register={formDriver.register}
                label='Nombre'
                placeholder='Pekkin'
                messageErrors={formDriver.formState.errors}
              ...
              <GenericSelect
                ...
                defaultValue={String(driver.isActive)}
                tabIndex={2}
                fieldControlled={{ control: formDriver.control, rules: meterModelRules.isActive }}
                items={[
                  {
                    label: 'Activo',
                    value: true
        ...
        <DialogFooter className='flex flex-col gap-y-4'>
          ...
          <Button
            type='button'
            isLoading={loading.value}
            onClick={formDriver.handleSubmit(onSubmitFormMterModel)}
          >
          Editar Modelo
          </Button>
        </DialogFooter>
