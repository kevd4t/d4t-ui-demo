...
export const CitiesRowActions = ({ city }: { city: ICity }) => {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align='end'>
        ...
        <DropdownMenuItem onClick={() => router.push(`/ajustes/ciudades/${city.id}`)}>
          <IconEye className='mr-2 h-4 w-4' />
          Ver Ciudad
        </DropdownMenuItem>

        <DropdownMenuItem onClick={() => router.push(`/ajustes/ciudades/${city.id}/editar`)}>
        ...
        <DropdownMenuItem onClick={() => navigator.clipboard.writeText(city.id.toString())}>
...
export const ColumnSort = ({ column, columnLabel }: { column: Column<ICity>, columnLabel: string }) => {
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className='w-full text-right'>
        <CitiesRowActions city={row.original} />
      </div>
    )
  }
]
