  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align='end'>
        ...
        <DropdownMenuItem onClick={() => router.push(`/categorias/${category.id}`)}>
          <IconEye className='mr-2 h-4 w-4' />
          Ver Categoria
        </DropdownMenuItem>

        <DropdownMenuItem onClick={() => router.push(`/categorias/${category.id}/editar`)}>
...
export const categoryColumns: ColumnDef<ICategory>[] = [
  ...
  },
  {
    ...
    header: 'Titulo',
    cell: ({ row: { original } }) => {
      return <div className='pl-4 w-full'>{original.title}</div>
    }
  },
  {
    id: 'Descripcion',
